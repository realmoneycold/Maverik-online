...
from actions import execute_action, monitor_build, _generate_project_name
...
- When asked to "pull it up" or "show me" an article you just summarized — use [ACTION:BROWSE] with NO URL (just `[ACTION:BROWSE] `). This will automatically open the exact article you were just talking about!
...
When asked to create or write to a file, DO NOT recite the file contents aloud in your spoken response! Just confidently confirm that it was written.
...
- [ACTION:BROWSE] url — when user explicitly asks you to "pull it up on my screen" or "show me the website". This will physically open a Chrome window on their computer. Do NOT use this just to search for information. If you want to pull up the article you just summarized, leave the url completely blank (e.g. `[ACTION:BROWSE] `) and it will open the recent article automatically!
- [ACTION:RESEARCH] search query — when user wants to search the web, get the latest news, or find out information. MAVERIK will instantly search the live web and summarize the results for you. Use this for ANY question requiring internet access.
- [ACTION:SYSTEM] command — when the user wants to run a simple, basic system command (like making a folder, deleting a file, etc). E.g. "create a folder named JarvisTest on Desktop" -> [ACTION:SYSTEM] mkdir -p ~/Desktop/JarvisTest. Do NOT use this for writing files or complex file management.
- [ACTION:MAVERIK] task description — when the user asks you to perform complex OS tasks, organize files, manage messy directories, or write scripts. This hands the task over to your advanced Agent Brain which can reason and execute multi-step logic. E.g. "organize my desktop files" -> [ACTION:MAVERIK] Organize the files on the ~/Desktop directory into categorized folders.
- [ACTION:WRITE_FILE] filepath ||| content — when the user wants to create a file or write text to a file. Provide the absolute or home-relative path and the content. E.g. [ACTION:WRITE_FILE] ~/Desktop/JarvisTest/hello.txt ||| Hello, World! I am JARVIS.
...
def extract_action(response: str) -> tuple[str, dict | None]:
    ...
    match = _action_re.search(
        r'\[ACTION:(BUILD|BROWSE|RESEARCH|OPEN_TERMINAL|PROMPT_PROJECT|ADD_TASK|ADD_NOTE|COMPLETE_TASK|REMEMBER|CREATE_NOTE|READ_NOTE|SCREEN|SYSTEM|WRITE_FILE|MAVERIK)\]\s*(.*?)$',
...
async def _execute_build(target: str, ws=None):
    """Execute a build action via MAVERIK."""
    try:
        log.info(f"Build started via MAVERIK: {target}")
        intent = {"action": "system_command", "target": target}
        result = await execute_action(intent)
        if ws and result.get("success"):
            try:
                notify_text = strip_markdown_for_tts(result["confirmation"])
...
async def _execute_browse(target: str, ws=None):
    """Execute a browse action by physically opening the URL in the local browser."""
    import webbrowser
    from urllib.parse import quote
    try:
        log.info(f"Opening browser for: {target}")
        
        # Determine the URL to open
        if not target.strip():
            # Check if we have recent research results to open
            if _last_research_results:
                first_result = _last_research_results[0]
                url = first_result.get("url") or first_result.get("href", "https://www.google.com")
                log.info(f"Opening first research result: {url}")
            else:
                url = "https://www.google.com"
        elif target.startswith("http") or ("." in target and len(target.split()) == 1):
            url = target if target.startswith("http") else f"https://{target}"
        else:
            url = f"https://www.google.com/search?q={quote(target)}"
            
        # Open in default browser (Chrome/Firefox/etc)
        webbrowser.open(url)
        
        if ws:
            try:
                notify_text = "I've pulled that up on your screen, sir."
                audio = await synthesize_speech(notify_text)
                if audio:
                    await ws.send_json({"type": "status", "state": "speaking"})
                    await ws.send_json({"type": "audio", "data": base64.b64encode(audio).decode(), "text": notify_text})
...
async def _execute_research(target: str, ws=None):
    """Execute web research directly using DuckDuckGo — bypasses MAVERIK brain entirely."""
    global _last_research_results
    try:
        # Sanitize empty targets
        query = target.strip() if target and target.strip() else "latest technology and AI news today"
        log.info(f"Research started (direct DDG search): {query}")

        # Step 1: Search DuckDuckGo for real news articles
        ...
        if not results:
            log.warning(f"DDG search returned no results for: {query}")
            fallback_text = f"I searched the web for {query} but didn't find any relevant results, sir."
            if ws:
                try:
                    audio = await synthesize_speech(fallback_text)
                    if audio:
                        await ws.send_json({"type": "status", "state": "speaking"})
                        await ws.send_json({"type": "audio", "data": base64.b64encode(audio).decode(), "text": fallback_text})
                        await ws.send_json({"type": "status", "state": "idle"})
                except Exception:
                    pass
            return

        # Store results for later browsing (so user can say "pull that up")
        _last_research_results = results
        log.info(f"DDG returned {len(results)} results")

        # Format results for the summarizer
        formatted = ""
        for i, r in enumerate(results, 1):
            title = r.get("title", "")
            body = r.get("body", "")
            url = r.get("url") or r.get("href", "")
            formatted += f"{i}. {title}: {body} (Source: {url})\n"

        # Step 2: Summarize via Groq Llama 3.3 (plain text, no tools = no XML bug)
        summary_text = ""
        try:
            groq_key = os.getenv("GROQ_API_KEY", "")
            async with httpx.AsyncClient(timeout=30.0) as http:
                groq_resp = await http.post(
                    ...
                    json={
                        ...
                        "temperature": 0.3,
                        "messages": [
                            {"role": "system", "content": "You are JARVIS, a voice assistant. Summarize these news search results into a spoken briefing of 3-5 sentences. Be specific — include names, numbers, dates, and key facts from the results. NO markdown, NO links, NO formatting. Speak naturally as if briefing someone verbally. End with something like 'Would you like me to pull up any of these articles, sir?'"},
                            {"role": "user", "content": f"The user asked about: '{query}'\n\nSearch results:\n{formatted}"}
                        ]
                    }
                )
                if groq_resp.status_code == 200:
                    ...
                    log.info(f"Groq summary generated: {summary_text[:100]}...")
                else:
                    log.error(f"Groq summary failed: {groq_resp.status_code} {groq_resp.text[:200]}")
                    raise Exception(f"Groq API returned {groq_resp.status_code}")
        except Exception as e:
            log.error(f"Summary LLM failed: {e}")
            # Fallback: manually construct a summary from titles
            titles = [r.get("title", "") for r in results if r.get("title")]
            summary_text = f"Here's what I found about {query}. " + ". ".join(titles[:3]) + ". Would you like me to open any of these in Chrome, sir?"

        # Step 3: Speak the summary to the user
        summary_text = strip_markdown_for_tts(summary_text)
        if ws:
            try:
                audio = await synthesize_speech(summary_text)
                if audio:
                    await ws.send_json({"type": "status", "state": "speaking"})
                    await ws.send_json({"type": "audio", "data": base64.b64encode(audio).decode(), "text": summary_text})
                    await ws.send_json({"type": "status", "state": "idle"})
                    log.info(f"JARVIS/Research: {summary_text}")
            except Exception:
                pass
        ...
        if ws:
            try:
                audio = await synthesize_speech("My apologies sir, the research task encountered an error.")
                if audio:
                    await ws.send_json({"type": "audio", "data": base64.b64encode(audio).decode(), "text": "Research error."})
            except Exception:
                pass
...
async def _execute_prompt_project(project_name: str, prompt: str, work_session: WorkSession, ws, dispatch_id: int = None, history: list[dict] = None, voice_state: dict = None):
    ...
    try:
        ...
        # Use MAVERIK AgentBrain instead of claude -p
        log.info(f"Dispatching to MAVERIK Agent Brain for {project_name}: {prompt[:80]}")
        dispatch_registry.update_status(dispatch_id, "building")
        
        # Format the intent for MAVERIK
        intent = {"action": "system_command", "target": f"In project '{project_name}': {prompt}"}
        result_dict = await execute_action(intent)
        full_response = result_dict.get("confirmation", "Execution finished.")
...
async def synthesize_speech(text: str) -> Optional[bytes]:
    ...
    if not api_key:
        log.warning("DEEPGRAM_API_KEY not set, skipping TTS")
        return None
    ...
    try:
        async with httpx.AsyncClient(timeout=45.0) as http:
            ...
            if response.status_code == 200:
                ...
                log.error(f"Deepgram TTS error {response.status_code}: {response.text}")
                return None
    except Exception as e:
        log.error(f"Deepgram TTS exception: {e}")
        return None
...
async def _do_calendar_lookup() -> str:
    """Slow calendar fetch — stubbed for Linux."""
    return "Calendar access is currently disabled."


async def _do_mail_lookup() -> str:
    """Slow mail fetch — stubbed for Linux."""
    return "Mail access is currently disabled."
                        ...
                        if not anthropic_client:
                            ...
                            if embedded_action:
                                ...
                                if embedded_action["action"] == "build":
                                    ...
                                    asyncio.create_task(_execute_browse(embedded_action["target"], ws=ws))
                                elif embedded_action["action"] == "research":
                                    asyncio.create_task(_execute_research(embedded_action["target"], ws=ws))
                                    ...
                                    target = embedded_action["target"]
                                    log.info(f"Stub create note: {target}")
                                elif embedded_action["action"] == "screen":
                                    asyncio.create_task(_lookup_and_report("screen", _do_screen_lookup, ws, history=history, voice_state=voice_state))
                                elif embedded_action["action"] == "system":
                                    async def _run_system_cmd(cmd, _ws):
                                        try:
                                            proc = await asyncio.create_subprocess_shell(cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
                                            stdout, stderr = await proc.communicate()
                                            audio = await synthesize_speech("Command executed successfully, sir." if proc.returncode == 0 else "There was an error executing that command, sir.")
                                            if audio:
                                                await _ws.send_json({"type": "audio", "data": base64.b64encode(audio).decode(), "text": "Command executed."})
                                        except Exception:
                                            pass
                                    asyncio.create_task(_run_system_cmd(embedded_action["target"].strip(), ws))
                                    ...
                                    if "|||" in target:
                                        ...
                                        log.warning("WRITE_FILE missing ||| delimiter")
                                elif embedded_action["action"] == "maverik":
                                    target = embedded_action["target"]
                                    async def _run_maverik(_target, _ws):
                                        try:
                                            intent = {"action": "system_command", "target": _target}
                                            result = await execute_action(intent)
                                            summary = result.get("confirmation", "Task completed.")
                                            audio = await synthesize_speech("The MAVERIK brain has completed the operation, sir.")
                                            if audio:
                                                await _ws.send_json({"type": "audio", "data": base64.b64encode(audio).decode(), "text": summary[:150]})
                                        except Exception as e:
                                            log.error(f"MAVERIK action failed: {e}")
                                    asyncio.create_task(_run_maverik(target, ws))
                                elif embedded_action["action"] == "read_note":
                                    msg = "Note reading is disabled, sir."
                                    async def _stub_read_note(_ws):
                                        audio = await synthesize_speech(strip_markdown_for_tts(msg))
                                        if audio and _ws:
                                            try:
                                                ...
                                                pass
                                    asyncio.create_task(_stub_read_note(ws))
